import { useState } from 'react'
import { Link } from 'react-router-dom'
import { useI18n } from '../i18n/LanguageContext'
import { getLanguageLocale } from '../i18n/translations'
import '../styles/mobile-event-card.css'

function formatEventDate(eventDate, fallback, languageCode) {
  if (!eventDate) {
    return fallback
  }

  const parsedDate = new Date(`${eventDate}T00:00:00`)
  if (Number.isNaN(parsedDate.getTime())) {
    return fallback || eventDate
  }

  return parsedDate.toLocaleDateString(getLanguageLocale(languageCode), {
    month: 'short',
    day: 'numeric',
  })
}

function getCategoryColor(category) {
  const colors = {
    'Opening': 'green',
    'General': 'purple',
    'Sports': 'orange',
    'Music': 'pink',
    'Academic': 'indigo',
    'Culture': 'teal',
  };
  return colors[category] || 'blue';
}

function getStatusColor(status) {
  const colors = {
    'Joined': 'green',
    'Opening': 'blue',
    'Waitlisted': 'orange',
    'Pending': 'gray',
  };
  return colors[status] || 'gray';
}

function MobileEventCard({
  event,
  currentUser = null,
  onToggleLike = null,
  isLikePending = false,
}) {
  const { languageCode, t } = useI18n();
  const [isLikeLoading, setIsLikeLoading] = useState(false);
  
  const dateText = formatEventDate(event.eventDate, event.date, languageCode);
  const categoryColor = getCategoryColor(event.category);
  const statusColor = getStatusColor(event.isJoined ? 'Joined' : 'Opening');
  const canToggleLike = Boolean(currentUser?.id) && typeof onToggleLike === 'function';

  async function handleLikeToggle(clickEvent) {
    clickEvent.preventDefault()
    clickEvent.stopPropagation()

    if (!canToggleLike || isLikeLoading || isLikePending) {
      return
    }

    setIsLikeLoading(true)

    try {
      await onToggleLike(event)
    } catch (error) {
      console.error('Could not update event like', error)
    } finally {
      setIsLikeLoading(false)
    }
  }

  const statusText = event.isJoined ? t('eventCard.joined') : t('eventCard.opening')
  const hasLikesCount = typeof event.likesCount === 'number'

  return (
    <article className="mobile-event-card">
      <Link to={`/events/${event.id}`} className="mobile-event-card__link">
        <div className="mobile-event-card__content">
          {/* Thumbnail */}
          <div className="mobile-event-card__thumbnail">
            <img 
              src={event.image} 
              alt={event.title}
              className="mobile-event-card__image"
              loading="lazy"
            />
          </div>

          {/* Main content */}
          <div className="mobile-event-card__main">
            {/* Title */}
            <h3 className="mobile-event-card__title">{event.title}</h3>

            {/* Badges */}
            <div className="mobile-event-card__badges">
              {event.category && (
                <span className={`mobile-event-card__badge mobile-event-card__badge--${categoryColor}`}>
                  {event.category}
                </span>
              )}
              <span className={`mobile-event-card__badge mobile-event-card__badge--${statusColor}`}>
                {statusText}
              </span>
            </div>

            {/* Details */}
            <div className="mobile-event-card__details">
              <div className="mobile-event-card__detail">
                <svg viewBox="0 0 24 24" aria-hidden="true" className="mobile-event-card__icon">
                  <path
                    d="M7 2a1 1 0 0 1 1 1v1h8V3a1 1 0 1 1 2 0v1h1a3 3 0 0 1 3 3v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a3 3 0 0 1 3-3h1V3a1 1 0 0 1 1-1Zm12 8H5v7a3 3 0 0 0 3 3h8a3 3 0 0 0 3-3v-7ZM5 8h14V7a1 1 0 0 0-1-1H6a1 1 0 0 0-1 1v1Z"
                    fill="currentColor"
                  />
                </svg>
                <span>{dateText}</span>
              </div>
              
              <div className="mobile-event-card__detail">
                <svg viewBox="0 0 24 24" aria-hidden="true" className="mobile-event-card__icon">
                  <path
                    d="M12 2.75A6.25 6.25 0 0 0 5.75 9c0 4.86 5.18 10.83 5.4 11.08a1.12 1.12 0 0 0 1.7 0c.22-.25 5.4-6.22 5.4-11.08A6.25 6.25 0 0 0 12 2.75Zm0 8.5A2.25 2.25 0 1 1 12 6.75a2.25 2.25 0 0 1 0 4.5Z"
                    fill="currentColor"
                  />
                </svg>
                <span>{event.location}</span>
              </div>
            </div>
          </div>

          {/* Like button */}
          {hasLikesCount && (
            <div className="mobile-event-card__actions">
              {canToggleLike ? (
                <button
                  type="button"
                  className={`mobile-event-card__like-button ${
                    event.isLiked ? 'mobile-event-card__like-button--liked' : ''
                  }`}
                  onClick={handleLikeToggle}
                  disabled={isLikeLoading || isLikePending}
                  aria-pressed={event.isLiked}
                  aria-label={`${event.isLiked ? t('eventDetails.likedAction') : t('eventDetails.likeAction')}: ${event.title}`}
                >
                  <svg viewBox="0 0 24 24" aria-hidden="true" className="mobile-event-card__heart-icon">
                    <path
                      d="M12 20.5 4.9 13.9A4.95 4.95 0 0 1 12 7.1a4.95 4.95 0 0 1 7.1 6.8L12 20.5Z"
                      fill="currentColor"
                    />
                  </svg>
                  <span className="mobile-event-card__like-count">{event.likesCount}</span>
                </button>
              ) : (
                <div className="mobile-event-card__like-display">
                  <svg viewBox="0 0 24 24" aria-hidden="true" className="mobile-event-card__heart-icon">
                    <path
                      d="M12 20.5 4.9 13.9A4.95 4.95 0 0 1 12 7.1a4.95 4.95 0 0 1 7.1 6.8L12 20.5Z"
                      fill="currentColor"
                    />
                  </svg>
                  <span className="mobile-event-card__like-count">{event.likesCount}</span>
                </div>
              )}
            </div>
          )}
        </div>
      </Link>
    </article>
  )
}

export default MobileEventCard
